...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $is_own_profile) {
    ...
    try {
        ...
        $message = __t('profile_success_update', $lang);
...
function formatRelativeTime(string $datetime, string $lang): string {
    ...
    if ($diff < 86400) return __t('time_just_now', $lang);
    if ($diff < 604800) {
        $days = floor($diff / 86400);
        return $days . ' ' . __t('time_day', $lang);
    }
...
<head>
  ...
  <title><?= $is_own_profile ? __t('profile_title', $lang) : htmlspecialchars($user['username']) . ' | AGRE Fitness' ?></title>
  ...
  <main class="max-w-4xl mx-auto p-4 pt-24 pb-8">
      ...
      <h1 class="text-3xl font-black tracking-tight mb-2">
        <?php if ($is_own_profile): ?>
          <span class="bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text text-transparent">
            <?= __t('profile_header_own', $lang) ?>
          </span>
        <?php else: ?>
          <span class="text-gray-400"><?= __t('profile_header_other', $lang) ?></span>
          <span class="text-white"><?= htmlspecialchars($user['username']) ?></span>
        <?php endif; ?>
      </h1>
      <p class="text-gray-500 text-sm">
        <?= $is_own_profile ? __t('profile_subtitle_own', $lang) : __t('profile_subtitle_other', $lang) ?>
      </p>
    </header>
        ...
        <div class="flex-1 text-center md:text-left">
          <div class="flex flex-col md:flex-row items-center gap-3 mb-3">
            ...
            <span class="gender-badge <?= ($user['gender'] ?? 'male') === 'female' ? 'gender-female' : 'gender-male' ?>">
              <?= ($user['gender'] ?? 'male') === 'female' ? '♀️ ' . __t('signup_gender_female', $lang) : '♂️ ' . __t('signup_gender_male', $lang) ?>
            </span>
          </div>
          ...
          <div class="flex items-center justify-center md:justify-start gap-4 mb-4">
            <div class="stat-card rounded-xl px-4 py-2 text-center">
              ...
              <p class="text-xs text-gray-500"><?= __t('profile_followers', $lang) ?></p>
            </div>
            <div class="stat-card rounded-xl px-4 py-2 text-center">
              <p class="text-xl font-bold text-white"><?= $followingCount ?></p>
              <p class="text-xs text-gray-500"><?= __t('profile_following', $lang) ?></p>
            </div>
            <div class="stat-card rounded-xl px-4 py-2 text-center">
              <p class="text-xl font-bold text-white"><?= count($profilePosts) ?></p>
              <p class="text-xs text-gray-500"><?= __t('profile_posts', $lang) ?></p>
            </div>
          </div>
            ...
            <div class="flex items-center justify-center md:justify-start gap-3">
              <button id="followBtn" 
                ...
                <?php if ($isFollowing): ?>
                  ...
                  <span id="followBtnText"><?= __t('profile_unfollow', $lang) ?></span>
                <?php else: ?>
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                  </svg>
                  <span id="followBtnText"><?= __t('profile_follow', $lang) ?></span>
                <?php endif; ?>
              </button>
    ...
    <?php if ($is_own_profile): ?>
      <form method="POST" enctype="multipart/form-data" class="space-y-4 animate-fade-in animate-delay-2">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div class="glass-card rounded-xl p-5">
            <label class="block text-xs font-bold uppercase tracking-wider text-blue-400 mb-2">
              <?= __t('profile_current_weight', $lang) ?>
            </label>
        ...
        <div class="glass-card rounded-xl p-5">
          <label class="block text-xs font-bold uppercase tracking-wider text-gray-500 mb-2">
            <?= __t('profile_bio', $lang) ?>
          </label>
          <textarea name="bio" rows="3" placeholder="<?= __t('profile_bio', $lang) ?>"
    ...
    <?php else: ?>
      ...
      <div class="glass-card rounded-xl p-6 mb-6 animate-fade-in animate-delay-2">
        <p class="text-gray-400 text-sm mb-4">
          📅 <?= __t('profile_member_since', $lang) ?> <?= date('d/m/Y', strtotime($user['created_at'] ?? 'now')) ?>
        </p>
        ...
        <div class="grid grid-cols-2 gap-4">
          <?php if ($user['age']): ?>
            <div class="stat-card rounded-xl p-4 text-center">
              ...
              <p class="text-xs text-gray-500"><?= __t('profile_age', $lang) ?></p>
            </div>
          <?php endif; ?>
          ...
          <?php if ($user['height']): ?>
            <div class="stat-card rounded-xl p-4 text-center">
              ...
              <p class="text-xs text-gray-500"><?= __t('profile_height', $lang) ?></p>
            </div>
          <?php endif; ?>
          ...
          <?php if ($user['current_weight']): ?>
            <div class="stat-card rounded-xl p-4 text-center">
              ...
              <p class="text-xs text-gray-500"><?= __t('profile_current_weight', $lang) ?></p>
            </div>
          <?php endif; ?>
          ...
          <?php if ($user['goal_weight']): ?>
            <div class="stat-card rounded-xl p-4 text-center">
              ...
              <p class="text-xs text-gray-500"><?= __t('profile_goal_weight', $lang) ?></p>
            </div>
          <?php endif; ?>
        </div>
      </div>
      ...
      try {
        ...
        if (data.success) {
          ...
          if (newIsFollowing) {
            ...
            btnText.textContent = '<?= __t('profile_unfollow', $lang) ?>';
            btn.innerHTML = `
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
              </svg>
